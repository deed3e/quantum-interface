...
const TokenListModal = ({
    ...
    const sortTokenList = useCallback(() => {
        ...
        for (const token of tokens) {
            ...
            if (Number(allTokenBalances[token.address]) < 1) {
    ...
    const ModalContent = (onClose: () => void) => {
        return (
            <ModalContentWrapper gap={'16px'}>
                <Row className="header" jus="space-between">
                ...
                <ModalContainer>
                    ...
                    />
                    <Question>
                        Common bases <img src={QuestionIcon} alt="" />
                    </Question>
                    <Row gap="10px" wrap="wrap">
                        {chainId &&
                            ALL_SUPPORTED_CHAIN_IDS.includes(
                                chainId as ChainId,
                            ) &&
                    ...
                    {/* <Hr /> */}
                    <SupTitle>Token name</SupTitle>
                    <WrapList>
                        {renderedTokenList.current.length > 0 ? (
                            renderedTokenList.current.map(
                                (token: Token, index: number) => {
                                    return (
                                        <TokenSelection
                                            key={index + 1}
