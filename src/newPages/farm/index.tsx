...
import {
    useFarmState,
    useSetIsListColumnView,
    useSetStakedOnly,
} from 'states/farm/hooks'
    ...
    const onCreate = async () => {
        try {
            if (
                ...
                !state.allocPoint ||
                !state.lpToken ||
                Number(state.allocPoint) <= 0
            )
                return
            setLoading(true)
            console.log('farming address', contract.address)
            const callResult = await contract.add(
                Number(state.allocPoint).toFixed(0),
                state.lpToken,
                true,
            )
        ...
        <AdminWrapper gap="10px">
            <Row al="center" gap="10px" jus="space-between">
                ...
                <TextInput
                    ...
                    onChange={(e) =>
                        setState({ ...state, allocPoint: e.target.value })
                    }
                    placeholder="0"
                />
            </Row>
            <Row al="center" gap="10px" jus="space-between">
                ...
                <TextInput
                    value={state.lpToken}
                    onChange={(e) =>
                        setState({ ...state, lpToken: e.target.value })
                    }
...
const Farm = () => {
    ...
    const [currentCalcPool, setCurrentCalcPool] = useState<{
        pool: IFarmingPool
        pairName: string
    }>()
    ...
    const getMoviesFromApi = async () => {
        try {
            let response = await fetch(
                'https://api.llama.fi/protocol/ancora-finance',
            )
        ...
        try {
            ...
            const callResult = currentUnstakePool.isStakePool
                ...
                : await contract.withdraw(
                      currentUnstakePool.pid,
                      currentUnstakePool.amount,
                  )
            initDataTransaction.setIsOpenWaitingModal(false)
            initDataTransaction.setIsOpenResultModal(true)
            ...
            <div className="wp-farm">
                    ...
                    <Row al="center" gap="10px">
                        <Row gap="10px" al="center">
                            <Row
                                ...
                                onClick={() => setIsListColumnView(true)}
                            >
                                {isListColumnView ? (
                                    <img
                                        src={ListColumnDark}
                            ...
                            <Row
                                ...
                                onClick={() => setIsListColumnView(false)}
                            >
                                {isListColumnView ? (
                                    <img
                                        src={ListRowWhite}
                ...
                {currentUnstakePool && (
                    <WithdrawModal
                        ...
                        setIsOpen={(bool: boolean) =>
                            !bool && setCurrentUnstakePool(undefined)
                        }
                ...
                {currentCalcPool && (
                    <Calculator
                        ...
                        setIsOpen={(bool: boolean) =>
                            !bool && setCurrentCalcPool(undefined)
                        }
...
const InfoWrapper = styled(Row)`
    width: 100%;
    border-bottom: 2px solid #364453;
    > div {
        position: relative;
        margin-bottom: 35px;
        padding-top: 20px;
