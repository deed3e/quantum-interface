...
const Pool = ({
    ...
    const pendingReward = useMemo(() => {
        ...
        const accTokenPerShare = divNumberWithDecimal(
            pool.accCakePerShare.toString(),
            18,
        )
    ...
    const Button = () => {
        const lpApproval = useTokenApproval(
            account,
            FARMING_ADDRESSES[chainId],
            lpToken,
        )
        const isInsufficientAllowance =
            Number(lpApproval.allowance) < Number(balance)
        ...
        <PoolWrapper isHidden={isHidden} isColumnView={isColumnView}>
            ...
            >
                <Row al="center" gap="10px">
                    ...
                    {!isColumnView && (
                        <div className="pool-name">{pairName}</div>
                    )}
                </Row>
                {isColumnView && (
                ...
                {!isColumnView && (
                    <>
                        ...
                        <Columns al="flex-start">
                            <div className="title_lable">Liquidity</div>
                            <div className="info-f">
                                ...
                                {pool.isStakePool ? 'ACR' : 'ACR-LP'}
                            </div>
                        </Columns>
                        <Columns al="flex-start">
                            <div className="title_lable">Your Liquidity</div>
                            <div className="info-f">
                                {Number(
                                    divNumberWithDecimal(
                                        pool.totalStaked.toString(),
                        ...
                        <Columns al="flex-start">
                            <div className="title_lable">Earned</div>
                            <div className="info-f">{pendingReward} ACR</div>
                        </Columns>

                        <Columns al="flex-start">
                            <div className="title_lable">Multiplier</div>
                            <div className="info-f">
                    ...
                    {isColumnView && (
                        <>
                            <Row jus="space-between">
                                ...
                                <div className="info-f">
                                    {(apr * 1000).toFixed(0) || 0}%
                                </div>
                            </Row>
                            <Row jus="space-between">
                                <div className="title_lable">Earn</div>
                                <div className="info-f">
                                    {ZKS_TOKEN[chainId || 59144].symbol} + Fees
                                </div>
                            </Row>
                        </>
                    )}
...
const PoolHeader = styled.div<{ isColumnView?: boolean }>`
    display: grid;
    align-items: center;
    grid-template-columns: 200px 100px 220px 220px 150px 150px auto;
    width: 100%;
    padding: 31px 50px;
